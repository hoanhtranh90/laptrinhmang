...
public class StorageServiceImpl implements StorageService {
    ...
    @Override
    public File saveFile(String id, String subFolder, Boolean isEncrypt, ISaveFileLogic iSaveFileLogic) {
    ...
    @Override
    public InputStream getInputStream(AttachDocument attachDocument) throws FileNotFoundException {
        return getInputStream(new FileInputStream(attachDocument.getFilePath()), H.isTrue(attachDocument.getIsEncrypt()));
    }

    @Override
    public void getFile(String filePath, Boolean isEncrypt, IGetFileLogic2 iGetFileLogic) {
        File decryptedFile = null;
        try {
            File sourceFile = new File(filePath);
            if (!isEncrypt) {
                iGetFileLogic.do_(sourceFile);
                return;
            }
            decryptedFile = getLocalFile(UUID.randomUUID().toString(), tempSubFolder);
            writeFile(getInputStream(new FileInputStream(sourceFile), isEncrypt), new FileOutputStream(decryptedFile));
            iGetFileLogic.do_(decryptedFile);
        } catch (Exception ex) {
            throw new StorageException("StorageException.convertToPdf", ex);
        } finally {
            try {
                ...
                logger.warn("getFile: delete decryptedFile error", ex);
            }
        }
    }
